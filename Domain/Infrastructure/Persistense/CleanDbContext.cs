...
namespace Persistense
{
    public partial class CleanDbContext : DbContext, ICleanDbContext 
    {
        ...
        protected CleanDbContext(DbContextOptions options, ICurrentUserAccessor currentUserAccessor) : base(options)
        {
            ...
            foreach (var entry in ChangeTracker.Entries<EntityBase>())
            {
                switch (entry.State)
                {
                    case EntityState.Added:
                        ...
                        entry.Entity.CreatedProgram = this._currentUserAccessor.Program;
                        entry.Entity.UpdatedDate = DateTime.Now;
                        entry.Entity.UpdatedBy = this._currentUserAccessor.UserName;
                        entry.Entity.UpdatedProgram = this._currentUserAccessor.Program;
                        break;
            ...
            foreach (var entry in ChangeTracker.Entries<EntityBase>())
            {
                switch (entry.State)
                {
                    case EntityState.Added:
                        ...
                        entry.Entity.CreatedProgram = programCode;
                        entry.Entity.UpdatedDate = DateTime.Now;
                        entry.Entity.UpdatedBy = userName;
                        entry.Entity.UpdatedProgram = programCode;
                        break;
                    case EntityState.Modified:
                        entry.Entity.UpdatedDate = DateTime.Now;
                        entry.Entity.UpdatedBy = userName;
