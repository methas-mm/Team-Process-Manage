...
namespace Infrastructure.Services
{
    ...
    public class ContentService : IContentService
    {
        ...
        public async Task CreateSubDirectory(long id, string subDirectory, CancellationToken cancellation)
        {
            var content = await this.FindById(id, cancellation);
            if (content != null && !content.ValidatePath)
            {
                try
                {
                    var oldPath = Path.Combine(this._sharedPath, content.Path);
                    var fileName = Path.GetFileName(content.Path);
                    var directory = Path.GetDirectoryName(content.Path);
                    var newPath = Path.Combine(this._sharedPath, directory, subDirectory, fileName);

                    if (File.Exists(oldPath))
                    {
                        Directory.CreateDirectory(Path.Combine(this._sharedPath, directory, subDirectory));
                        File.Move(oldPath, newPath);
                        _context.Set<SuContent>().Attach(content);
                        content.Path = Path.Combine(directory, subDirectory, fileName);
                        content.ValidatePath = true;
